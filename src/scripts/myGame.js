...
class Zombie extends Phaser.GameObjects.Sprite {
    constructor(pScene, x, y, myTextureKey) {
      super(pScene, x, y, myTextureKey);
    ...
    pScene.time.addEvent({
        ...
        loop: true,
        callback: () => {
            if (pScene.physics.overlap(myGlobal.duck, this.vision)){
                this.vision.body.debugBodyColor = 0xff9900;
                pScene.physics.moveTo(this, myGlobal.duck.x, myGlobal.duck.y);
            } else {
    ...
    pScene.time.addEvent({
        ...
        loop: true,
        callback: () => {
            if (pScene.physics.overlap(myGlobal.duck, this)) {
                this.body.stop();
            }
        }
    });
...
function create() {
    ...
    myGlobal.zombie = myScene.add.existing(new Zombie(myScene, 200, 100, 'charactersWeapons'));
